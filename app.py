...
with col1:
    ...
    if mode == "自己上傳照片":
        ...
        prompt = st.text_area("模特兒描述", value="A professional studio portrait of a beautiful Asian female model, front view, wearing a plain white t-shirt, clean background")
...
if st.button("✨ ✨ 開始魔法換裝 ✨ ✨", type="primary", use_container_width=True):
    if st.session_state["human_img"] and st.session_state["cloth_img"]:
        with st.spinner("🚀 AI 試衣間合成中...這一步大約 30-50 秒"):
            try:
                # 【核心修正】補上絕對正確的版本號，避免 404 錯誤
        ...
        st.warning("⚠️ 請確認人物與衣服都已經在上面顯示了喔！")
